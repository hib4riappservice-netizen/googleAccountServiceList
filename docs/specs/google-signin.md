...
- **`tests/e2e/auth.spec.ts` は実アカウントでの同意完了までは自動化していない**
  ...
  Google認可エンドポイントへの遷移・エラー出し分けのみ検証。
- SEC-05（他人のリソース操作の失敗テスト）は**該当なし**。DALは現状セッション読み取りのみで
  所有権判定の対象リソースが無いため。

## 手動確認（2026-08-15、Google Cloud側の認証情報発行後に実施）

`scripts/setup-google-oauth.sh` でGoogle Cloud側の認証情報を発行し、`pnpm dev` で実施。
受け入れ条件の1〜3を実アカウントで確認済み: 未サインイン表示 → サインイン
（gmail.readonly含む同意画面 → 許可）→ 表示名+サインアウトボタン表示 → サインアウトで
未サインイン表示に復帰。すべて緑。

途中、クライアントシークレットの貼り付けミスで `invalid_client` エラーが発生したが、
`app/auth/error` の想定外エラー処理（エラーID発行・内部コード非表示）が正しく機能し、
安全側に倒れることを実地でも確認できた。
